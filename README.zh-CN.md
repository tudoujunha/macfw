...
### 推荐方式：通过 PyPI 用 `pipx` 安装

对于命令行工具来说，`pipx` 是最合适的默认安装方式。它会把 `macfw` 放到独立环境里，同时仍然把命令暴露到你的 shell `PATH`。
...
### 备选方式：通过 PyPI 用 `pip` 安装

如果你更习惯普通 Python 环境，也可以直接：
